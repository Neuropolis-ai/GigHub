...
**Приоритет: Высокий**
- [ ] Внедрить `NextSeo` компонент на страницах сервисов
- [ ] Динамические метатеги:
  - [ ] `title`: "{service.title} - лучший ИИ-сервис | GigHub"
  - [ ] `description`: сокращенное описание сервиса (до 160 символов)
  - [ ] `keywords`: категория + функциональность сервиса
  - [ ] `canonical`: полный URL со slug
- [ ] Open Graph теги:
  - [ ] `og:title`, `og:description`, `og:image` (обложка сервиса)
  - [ ] `og:type`: "website" 
  - [ ] `og:site_name`: "GigHub - Каталог ИИ-сервисов"
- [ ] Twitter Card теги:
...
**Приоритет: Средний**
- [ ] Парсинг существующего поля `faq_ru`
- [ ] Внедрение `FAQPageJsonLd` компонента
- [ ] Структурированное отображение FAQ в UI

### 6. 🔴 Оптимизация изображений для SEO
**Приоритет: Средний**
- [ ] Добавить alt-теги для всех изображений:
  - [ ] Логотипы: "Логотип {service.title}"
  - [ ] Обложки: "Скриншот интерфейса {service.title}"
